...
WELCOME_MESSAGE = os.getenv(
    "WELCOME_MESSAGE",
    "Bem-vindo à comunidade YVORA de experiências gastronômicas",
)
...
PARTNERS_SHEET_ID = os.getenv(
    "PARTNERS_SHEET_ID",
    "1s1dGQZGG1A5M-6yD5fyggy4otmS8zQFl_GRBOiU32CI",
)
...
def get_media() -> dict:
    ...
    try:
        ...
        for item in data.get("data", []) or []:
            ...
            result["items"].append(
                {
                    "caption": (item.get("caption") or "").strip()[:130],
                    "media_type": media_type,
                    "image": image,
                    "permalink": item.get("permalink") or PROFILE_URL,
                    "like_count": likes,
...
def get_partners() -> list[dict]:
    try:
        csv_url = (
            f"https://docs.google.com/spreadsheets/d/{PARTNERS_SHEET_ID}"
            f"/gviz/tq?tqx=out:csv&sheet={PARTNERS_SHEET_NAME}"
        )
    ...
    cards = "".join(
        [
            ...
            <div class="partner-card">
              <img src="{esc(item["image"])}" alt="{esc(item["name"])}" title="{esc(item["name"])}">
            </div>
            """
...
def render():
    ...
    latest_html = (
        "".join(post_card(item, "Último post") for item in latest)
        or f'<div class="empty">Sem posts carregados.<br><small>{esc(error_msg)}</small></div>'
    )

    top_html = (
        "".join(post_card(item, "Maior interação") for item in top_posts)
        or f'<div class="empty">Sem dados de interação carregados.<br><small>{esc(error_msg)}</small></div>'
    )

    logo_html = (
        f'<img src="{logo_uri}" class="logo-img" alt="YVORA">'
        if logo_uri
        else '<div class="logo-text">YVORA</div>'
    )
    ...
    burst_html = (
        "".join(
            [
                f'<div class="celebration-burst w{i}">{celebration_items[i]}</div>'
                for i in range(len(celebration_items))
            ]
        )
        if should_burst
        else ""
    ...
    welcome_icons = (
        f"<span>🍷</span>"
        f"<span class='inline-br-flag'>{BRAZIL_FLAG_SVG}</span>"
        f"<span>🍽️</span>"
    )
    ...
    welcome_html = (
        ...
        <div class="welcome-toast">
          <div class="welcome-kicker">Novo seguidor</div>
          <div class="welcome-title">{esc(WELCOME_MESSAGE)}</div>
          <div class="welcome-icons">{welcome_icons}</div>
        </div>
        """
        if should_burst
        else ""
    ...
    if delta == 1:
        change_html = '<div class="change positive">+1 novo seguidor</div>'
    elif delta > 1:
        change_html = f'<div class="change positive">+{delta} novos seguidores</div>'
    else:
        change_html = ""
...
.partner-bar {
  ...
  align-items: center;
  overflow: hidden;
  box-shadow: 0 10px 24px rgba(57, 43, 35, .07);
}
...
.card {
  ...
  border-radius: 24px;
  padding: 24px;
  box-shadow: 0 12px 30px rgba(57, 43, 35, .08);
  position: relative;
  overflow: hidden;
}
...
.welcome-toast {
  ...
  border-radius: 24px;
  padding: 20px 26px;
  box-shadow: 0 22px 60px rgba(57, 43, 35, .22);
  animation: welcomeToast 7.2s ease-out forwards;
}
...
.celebration-burst {
  ...
  font-size: 46px;
  z-index: 999999;
  animation: celebrationFloat 7.8s ease-out forwards;
  pointer-events: none;
...
.celebration-burst .br-flag {
  width: 52px;
  height: 36px;
  border-radius: 6px;
}

.w0 { left: 6%; animation-delay: 0s; }
.w1 { left: 13%; animation-delay: .16s; }
.w2 { left: 21%; animation-delay: .32s; }
.w3 { left: 30%; animation-delay: .48s; }
.w4 { left: 39%; animation-delay: .64s; }
.w5 { left: 48%; animation-delay: .80s; }
.w6 { left: 57%; animation-delay: .96s; }
.w7 { left: 66%; animation-delay: 1.12s; }
.w8 { left: 75%; animation-delay: 1.28s; }
.w9 { left: 83%; animation-delay: 1.44s; }
.w10 { left: 91%; animation-delay: 1.60s; }
.w11 { left: 96%; animation-delay: 1.76s; }

@keyframes celebrationFloat {
  0% {
    transform: translateY(0) scale(.58) rotate(-8deg);
    opacity: 0;
  }
  12% {
    opacity: 1;
  ...
  100% {
    transform: translateY(-110vh) scale(1.24) rotate(12deg);
    opacity: 0;
  }
}
...
<div class="shell">
  ...
  <div class="header">
    <div class="brand">
      ...
      <div>
        <div class="title">{esc(BRAND_NAME)}</div>
        <div class="subtitle">@{esc(status.get("username"))}</div>
      </div>
    </div>
    ...
    <div class="pill">
      {esc(status.get("source"))} · atualizado às {current_time.strftime("%H:%M:%S")} · refresh {REFRESH_SECONDS}s
    </div>
  </div>
...
<div class="card">
  ...
  <div class="main-qr">
    <div class="main-qr-heading">Siga o <span>YVORA no Instagram</span></div>
    <img src="{instagram_qr}">
    <div class="main-qr-title">Aponte a câmera</div>
    <div class="main-qr-subtitle">e acompanhe o YVORA</div>
  </div>

  <div class="metrics">
    <div class="metric">
      <b>{int(status.get("media_count", 0))}</b>
  ...
  <div class="small-qrs">
    <div class="qr">
      <img src="{menu_qr}">
      <span>Menu Sensorial</span>
    </div>

    <div class="qr">
      <img src="{wine_qr}">
      <span>Wine Explorer</span>
    </div>
  </div>
...
<div class="stack">
  <div class="card">
    ...
    <div class="posts">{latest_html}</div>
  </div>

  <div class="card">
    <div class="section-title">Posts com maior interação</div>
    <div class="posts">{top_html}</div>
  </div>
</div>
"""

    full_html = css + header_html + left_html + right_html + "</div></div>"
