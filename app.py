...
def post_card(item: dict, label: str) -> str:
    ...
    link = esc(item.get("permalink") or PROFILE_URL)
    caption = esc(item.get("caption") or "Conteúdo YVORA")
    likes = int(item.get("like_count") or 0)
    comments = int(item.get("comments_count") or 0)
    media_type = "Reel" if item.get("media_type") == "VIDEO" else "Post"
    return f"""
    <a class="post" href="{link}" target="_blank">
      <img src="{image}" alt="Post YVORA">
      <div class="post-body">
        <div class="post-label">{esc(label)} · {media_type}</div>
        <div class="post-stats">♥ {likes} · 💬 {comments}</div>
...
<div class="shell">
  <div class="header">
    <div class="brand">
      ...
      <div>
        ...
        <div class="subtitle">{esc(BRAND_SUBTITLE)} · @{esc(status.get('username', 'yvora.restaurante'))}</div>
      </div>
    </div>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="handle">@{esc(status.get('username', 'yvora.restaurante'))}</div>
      <div class="main-qr">
        <img src="{instagram_qr}">
        <div class="main-qr-title">Siga nosso Instagram</div>
        <div class="main-qr-subtitle">Aponte a câmera e acompanhe o YVORA</div>
      </div>
      <div class="cta">{esc(FOLLOW_CTA)}</div>
      <div class="metrics">
        <div class="metric"><b>{int(status.get('media_count', 0))}</b><span>publicações</span></div>
        <div class="metric"><b>{len(media)}</b><span>posts lidos</span></div>
      </div>
      <div class="small-qrs">
        <div class="qr"><img src="{menu_qr}"><span>Menu Sensorial</span></div>
        <div class="qr"><img src="{wine_qr}"><span>Wine Explorer</span></div>
      </div>
